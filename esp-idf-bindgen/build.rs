...
fn main() -> Result<(), Box<dyn Error>> {
  ...
  let (idf_target, linker) = match env::var("TARGET")?.as_ref() {
    ...
    "xtensa-esp32-espidf" => {
      println!(r#"cargo:rustc-cfg=target_device="esp32""#);
      ("esp32".to_string(), "xtensa-esp32-elf-ld".to_string())
    },
  ...
  let idf_path = PathBuf::from(env::var("IDF_PATH").unwrap_or("./esp-idf-v4.3.4/".to_string())).canonicalize().expect("failed to canonicalize idf_path");
  ...
  let component_includes =
    globwalk::GlobWalkerBuilder::from_patterns(
      &idf_path,
      &["components/*/include", "components/*/include/esp32/include", "components/*/*/*/*/include"],
    )
    .build()?
  ...
  let component_additional_includes = globwalk::GlobWalkerBuilder::from_patterns(
      &idf_path,
      &["components/*/component.mk", "components/*/esp32/components/component.mk"],
    )
    .build()?
    .filter_map(Result::ok)
    .flat_map(|makefile| {
